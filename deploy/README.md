...
| 远端机器 | Linux + systemd；示例远端为 `<your-deploy-host>`。 |
...
```bash
scp deploy/systemd/* <your-deploy-host>:/tmp/comics-deploy/ && \
ssh <your-deploy-host> 'sudo cp /tmp/comics-deploy/*.service /tmp/comics-deploy/*.timer /etc/systemd/system/ && sudo systemctl daemon-reload && sudo systemctl enable --now comics-crawler.timer'
```
